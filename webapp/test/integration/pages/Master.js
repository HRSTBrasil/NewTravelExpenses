		...
		onTheMasterPage : {
			...
			actions : {
				...
				iSortTheListOnName : function () {
					return this.iChooseASorter("sortButton", "Sort By <externalName>");
				},
				iSortTheListOnUnitNumber : function () {
					return this.iChooseASorter("sortButton", "Sort By <cust_valor_total>");
				},

				iFilterTheListOnUnitNumber : function () {
					return this.iMakeASelection("filterButton", "<cust_valor_total>", "<100 <cust_currency>");
				},

				iGroupTheList : function () {
					return this.iChooseASorter("groupButton", "<cust_valor_total> Group");
				},
				...
				iRememberAnIdOfAnObjectThatsNotInTheList : function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet : "cust_travel_expenses",
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("externalCode") === oObject.externalCode;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].externalCode;
									} else {
										sCurrentId = aItemsNotInTheList[0].externalCode;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("cust_travel_expenses", {
										externalCode : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("externalCode"),
						title: oBindingContext.getProperty("externalName")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<cust_travel_expenses> (0)"}),
						success: function () {
				...
				theListShouldBeFilteredOnUnitNumber : function () {
					return this.theListShouldBeFilteredOnFieldUsingComparator("cust_valor_total", 100);
				},


				theListShouldBeSortedAscendingOnUnitNumber : function () {
					return this.theListShouldBeSortedAscendingOnField("cust_valor_total");
				},

				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("externalName");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all cust_travel_expenses to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<cust_travel_expenses> (" + iExpectedLength + ")"}),
								success : function () {
